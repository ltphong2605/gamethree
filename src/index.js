...
var camera, scene, renderer, spotlight,totalElapse;
...
var score;
export var phase; // -1 = pre-game screen, 0 = pause, 1 = in-game, (post-game = pre-game)
var composer;
...
//   model.position.setScalar(0,0.01,0)
...
machine.start()
  var sxs=[]	
  sxs.push(new THREE.Vector3(  0.000, 0.000, -0.150 ))
  sxs.push(new THREE.Vector3( -0.017, 0.000,  0.040 ))
  sxs.push(new THREE.Vector3(  0.017, 0.000,  0.040 ))
  sxs.push(new THREE.Vector3(  0.000, 0.013,  0.020 ))
    ...
    var triGeo = new THREE.BufferGeometry().setFromPoints(sxs);
     triGeo.computeVertexNormals()

	triangle = new THREE.Mesh(triGeo, new THREE.MeshBasicMaterial({ color: 0xffffff }));
...
export function gameReset() {
  ...
  var k = Math.PI - ( camera.rotation.z % ( 2*Math.PI ));
...
function newLevel() {
  // Update level vars
	...
	level++;
  document.getElementById('score').innerHTML=`New Level ${level}`
stnBounce = 2;  // bounce       { none,     some,       too much }
 stnBlock = Math.floor(Math.random() * 2);;   // blocks       { normal,   falling,    pop up }
...
function animate() {
  ...
  // Game state
    ...
    // In-game
      ...
      totalElapse += elapse;
      uiScore.innerHTML = `Level ${level+1} - ${(Math.round(totalElapse / 100) / 10).toString()}`
...
window.pauseGameScreen=pauseGameScreen;
function custompauseGameScreen() {
	phase=1;
  if( phase == 0 ) {
		phase = 1;
        uiPause.style.visibility = "hidden";
...
window.custompauseGameScreen=custompauseGameScreen;
